...
class AdvancedSimilarCaseSearch:
    def __init__(self, csv_path):
        """고급 유사사례 검색기 초기화"""
        self.df = pd.read_csv(csv_path, encoding='cp949', low_memory=False)
    ...
    def create_similarity_matrix(self):
        ...
        for idx, row in self.df.iterrows():
            text = f"{row.get('사고설명', '')} {row.get('업종한글명', '')}"
        ...
        self.vectorizer = TfidfVectorizer(
            ...
            stop_words=None,
            ngram_range=(1, 2),  # n-gram 범위 줄임
            min_df=3,  # 최소 문서 빈도 증가
            max_df=0.9
        )
